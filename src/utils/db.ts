...
class DatabaseService {
    ...
    async upsertRepos(repos: Repo[]): Promise<void> {
        ...
        while (i < sortedRepos.length) {
            ...
            if (cursorId === repo.id) {
                ...
                const mergedDescCn = repo.description_cn !== undefined ? repo.description_cn : existing.description_cn;
                ...
                await cursor.update({
                    ...
                    sync_status: repo.sync_status || 'pending',
                    last_updated: Date.now(),
                    translation_status: mergedDescCn ? 1 : 0
                });
                i++;
                cursor = await cursor.continue();
    ...
    async getUntranslatedRepos(limit: number = 50): Promise<Repo[]> {
        ...
        // 0 = untranslated
        return index.getAll(IDBKeyRange.only(0), limit);
    }
