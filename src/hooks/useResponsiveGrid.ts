...
export function useResponsiveGrid(containerRef: RefObject<HTMLElement>): GridDimensions {
    ...
    useEffect(() => {
        ...
        const calculate = () => {
            ...
            // 3. Recommended items per page (fill approx 2 viewports for smooth scrolling)
            // Must be multiple of columns to avoid jagged ends
            const targetRows = rowsInViewport * 2;
            const itemsPerPage = columns * targetRows;

            setDimensions({
                columns,
                rows: targetRows,
                itemsPerPage: Math.max(columns * 2, itemsPerPage) // Minimum 2 rows
            });
        };
