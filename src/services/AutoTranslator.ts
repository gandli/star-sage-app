    ...
    private async processQueue_(deadline: IdleDeadline) {
        ...
        if (deadline.timeRemaining() > 0 || deadline.didTimeout) {
            ...
            try {
                ...
                if (githubTotal > 0) {
                    const { count: currentTranslated } = await supabase
                        .from('user_stars')
                        .select('repos!inner(id)', { count: 'exact', head: true })
                        .eq('user_id', this.userId)
                        .eq('github_user', this.githubUser)
                        .not('repos.description_cn', 'is', null);

                    if (currentTranslated !== null && currentTranslated >= githubTotal) {
                        console.log(`[AutoTranslator] 100% Progress reached (${currentTranslated}/${githubTotal}). Sleeping deeply.`);
                        this.setProcessing(false);
                        // Significant delay for 100% progress
