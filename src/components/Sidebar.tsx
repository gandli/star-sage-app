...
const Sidebar: React.FC<SidebarProps> = ({
    ...
    const navItemClass = "flex items-center gap-3 px-6 py-2.5 mx-2 rounded-xl cursor-pointer transition-all duration-300 text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-black/5 dark:hover:bg-white/5 active:scale-[0.98] font-body";
                ...
                <div className={`mt-auto p-4 border-t border-[var(--border-main)] transition-all duration-300 ${collapsed ? 'items-center' : ''}`}>
                    <div className={`flex items-center gap-3 ${collapsed ? 'justify-center' : ''} p-2 rounded-xl bg-black/5 dark:bg-white/5`}>
                        <div className="relative">
                            {profile?.avatar_url ? (
                                <img src={profile.avatar_url} className="w-8 h-8 rounded-full border border-blue-500/30" alt={`${profile.username || 'User'} avatar`} />
                            ) : (
