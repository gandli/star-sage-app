...
const Sidebar: React.FC<SidebarProps> = ({
    ...
    const navItemClass = "flex items-center gap-3 px-6 py-2 mx-2 rounded-xl cursor-pointer transition-all duration-300 text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-black/5 dark:hover:bg-white/5 active:scale-[0.98] font-body text-left bg-transparent border-0";
            ...
            >
                <div className={`relative group/sidebar-header transition-all duration-300 ${collapsed ? 'h-24 flex items-center justify-center' : 'h-20 px-6 flex items-center justify-between'}`}>
                    <button
                        ...
                        className={cn("flex items-center gap-3 transition-all duration-300 cursor-pointer bg-transparent border-0", collapsed ? "absolute inset-0 flex items-center justify-center" : "")}
                        aria-label="StarSage Home"
                    >
                        <img
                            src={logo}
                            alt="Logo"
