...
const Cell = ({ columnIndex, rowIndex, style, repos, columnCount, token }: any) => {
    const index = rowIndex * columnCount + columnIndex;
    if (index >= repos.length) return null;

    const repo = repos[index];

    // Adjust for gap (24px = 1.5rem)
    const gutter = 24;
    ...
    return (
        <div style={{
            ...
            <RepoCard repo={repo} index={index} token={token} />
        </div>
    );
};
        ...
        <div className="w-full h-full pt-2">
            <AutoSizer>
                {({ height, width }) => {
                    const gutter = 24;
                    const minWidth = 280;
                    const columnCount = Math.floor((width) / (minWidth + gutter)) || 1;
                    ...
                    return (
                        <Grid
                            ...
                            width={width}
                            cellComponent={Cell}
                            cellProps={{ repos, columnCount, token }}
                        />
                    );
                }}
            </AutoSizer>
