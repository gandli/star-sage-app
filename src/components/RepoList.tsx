...
const RepoCard: React.FC<RepoCardProps> = ({ repo, index, token }) => {
    ...
    return (
        <motion.div
            ...
            className="group relative flex flex-col p-4 rounded-[1.75rem] premium-glass transition-all duration-500 hover:-translate-y-3 hover:shadow-[0_20px_60px_-15px_rgba(0,0,0,0.3),0_0_40px_-10px_rgba(59,130,246,0.3)] dark:hover:shadow-[0_20px_60px_-15px_rgba(0,0,0,0.5),0_0_40px_-10px_rgba(59,130,246,0.4)] content-visibility-auto overflow-hidden border-[var(--border-glass)] bg-[var(--bg-glass)] h-[260px]"
        >
            {/* Enhanced glow effect on hover with gradient border */}
            <div className="absolute inset-0 rounded-[1.75rem] bg-gradient-to-br from-blue-500/0 via-purple-500/0 to-pink-500/0 opacity-0 group-hover:from-blue-500/20 group-hover:via-purple-500/15 group-hover:to-pink-500/20 group-hover:opacity-100 transition-all duration-700" />
            <div className="absolute inset-0 rounded-[1.75rem] ring-1 ring-transparent group-hover:ring-blue-400/30 transition-all duration-500" />

            <div className="relative z-10 flex-1 flex flex-col">
                <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-3">
                        <div className="relative group/avatar">
                            <img src={repo.owner.avatar_url} alt={`${repo.owner.login} avatar`} className="w-8 h-8 rounded-2xl ring-2 ring-black/5 dark:ring-white/10 group-hover/avatar:scale-110 group-hover/avatar:ring-blue-400/40 transition-all duration-300" />
                        ...
                        <div className="flex flex-col">
                            <span className="text-[10px] font-black uppercase tracking-[0.2em] opacity-40 group-hover:opacity-100 group-hover:text-blue-500 transition-all duration-300">{repo.owner?.login || 'Unknown'}</span>
                ...
                >
                    <h3 className="font-bold text-xl line-clamp-1 group-hover:text-blue-500 group-hover:scale-[1.02] origin-left transition-all duration-300 uppercase">
                        {repo.name}
                    </h3>
                </a>
                ...
                {repo.topics && repo.topics.length > 0 && (
                    <div className="flex flex-wrap gap-2 mb-2 h-[26px] overflow-hidden">
                        {repo.topics.slice(0, 4).map(topic => (
                            <span
                                key={topic}
                                className="text-[9px] font-black uppercase tracking-wider px-2 py-0.5 rounded-lg bg-blue-500/5 text-blue-500/60 border border-blue-500/10 group-hover:bg-blue-500/20 group-hover:text-blue-500 group-hover:scale-105 group-hover:border-blue-500/30 transition-all duration-300"
                            >
                                {topic}
                            </span>
                        ))}
                ...
                <div className="mt-auto pt-3 border-t border-[var(--border-main)] group-hover:border-blue-500/20 transition-colors duration-300 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        {repo.language && (
                            <div className="flex items-center gap-2 group/lang">
                                <LanguageIcon name={repo.language} size={12} className="group-hover/lang:scale-110 transition-transform" />
                                <span className="text-[9px] font-black uppercase tracking-widest opacity-60 group-hover/lang:opacity-100 group-hover/lang:text-blue-500 transition-all">{repo.language}</span>
                            </div>
                        )}
                        <div className="flex items-center gap-1.5 group/stats">
                            <Star size={11} className="text-yellow-500 fill-yellow-500 group-hover/stats:scale-125 group-hover/stats:drop-shadow-[0_0_4px_rgba(234,179,8,0.6)] transition-all" />
                            <span className="text-[10px] font-black tabular-nums opacity-60 group-hover/stats:opacity-100 group-hover/stats:text-yellow-600 dark:group-hover/stats:text-yellow-400 transition-all">{(repo.stargazers_count || 0).toLocaleString()}</span>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                        <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="opacity-20 hover:opacity-100 transition-all duration-300 hover:scale-125 hover:drop-shadow-[0_0_6px_rgba(59,130,246,0.4)]"
                        ...
                        <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="opacity-20 hover:opacity-100 transition-all duration-300 hover:scale-125 hover:drop-shadow-[0_0_6px_rgba(33,193,154,0.4)]"
