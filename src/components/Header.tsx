    ...
    return (
        ...
        >
            ...
            <div className={`flex items-center gap-1.5 md:gap-3 transition-all duration-300 ${isSearchFocused ? 'flex-1 md:flex-initial' : 'flex-shrink-0'}`}>
                ...
                {/* Refresh Button - Always visible on desktop, hidden when search focused on mobile */}
                <button
                    onClick={onRefresh}
                    disabled={loading}
                    className={`h-10 w-10 flex items-center justify-center rounded-xl text-white shadow-lg transition-all hover:scale-105 active:scale-95 ${isSearchFocused ? 'hidden md:flex' : 'flex'} ${loading ? 'bg-blue-500 shadow-blue-500/20 cursor-wait' :
                        isTranslating ? 'bg-purple-500 shadow-purple-500/20 animate-pulse' :
                            'bg-blue-500 shadow-blue-500/20'
                        }`}
                    title={loading ? 'Syncing...' : isTranslating ? 'Translating descriptions...' : 'Sync Stars'}
                >
                    {loading ? (
