...
import { Moon, Sun, User, RefreshCw, Search, X, ChevronLeft, ChevronRight, SortDesc, SortAsc, Clock, Calendar, Star, Type, PanelLeft, Languages } from 'lucide-react';
...
const Header: React.FC<HeaderProps> = ({
    ...
    return (
        ...
        >
            ...
            <div className={`flex items-center gap-2 md:gap-4 font-black transition-all duration-300 ease-in-out whitespace-nowrap overflow-hidden ${isSearchFocused ? 'opacity-0 max-w-0 md:opacity-100 md:max-w-none md:overflow-visible' : 'opacity-100 max-w-[200px] xs:max-w-[300px] md:max-w-none'}`}>
                {/* Mobile Menu Toggle */}
                <button
                    onClick={onOpenMenu}
            ...
            <div className={`flex items-center gap-1.5 md:gap-3 transition-all duration-300 ml-auto justify-end ${isSearchFocused ? 'flex-1 md:flex-initial' : 'flex-1 md:flex-none'}`}>
                <div className={`relative flex items-center h-10 gap-2 pl-3 pr-1.5 rounded-xl bg-black/5 dark:bg-white/5 border border-black/5 dark:border-white/5 transition-all duration-300 ease-in-out w-full md:w-auto ${isSearchFocused ? 'max-w-full md:max-w-none md:w-64 lg:w-80 ring-2 ring-blue-500/50 border-blue-500/30' : 'max-w-[6rem] xs:max-w-[8rem] sm:max-w-[10rem] md:max-w-none md:w-56 lg:w-64'}`}>
                ...
                <div className={`hidden sm:flex items-center h-10 gap-2 md:gap-2.5 px-3 md:px-4 rounded-xl bg-black/5 dark:bg-white/5 border border-black/5 dark:border-white/5 group transition-all ${isSearchFocused ? 'md:hidden lg:flex' : ''}`}>
                    {profile?.avatar_url || config.resolvedUsername || (config.type === 'username' && config.value) ? (
                        <img
                            src={profile?.avatar_url || `https://github.com/${config.resolvedUsername || config.value}.png`}
                            alt={`${profile?.username || config.resolvedUsername || 'User'} profile`}
                    ...
                    <span className="hidden md:inline text-[10px] font-black uppercase tracking-widest max-w-[120px] truncate opacity-60 group-hover:opacity-100 transition-opacity">
                        {profile?.full_name || profile?.username || config.resolvedUsername || (config.type === 'username' ? config.value : null) || 'Analytic'}
                    </span>
                </div>
