                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 sm:p-12 overflow-hidden">
                    ...
                    >
                        ...
                        <div className="space-y-10">
                            <div className="flex p-1.5 rounded-2xl bg-black/[0.03] dark:bg-white/[0.03] border border-black/5 dark:border-white/5">
                                <button
                                    ...
                                    onClick={() => setTempConfig({
                                        type: 'username',
                                        value: config.type === 'username' ? tempConfig.value : (config.resolvedUsername || '')
                                    })}
                                >Username</button>
                                <button
                                    className={cn(
                                    ...
                                    onClick={() => setTempConfig({
                                        type: 'token',
                                        value: config.type === 'token' ? tempConfig.value : ''
                                    })}
                                >API Token</button>
                            </div>
                            ...
                            {/* Pagination Settings */}
                            <div className="space-y-4">
                                <label className="block text-[10px] font-black uppercase tracking-widest opacity-40">Items Per Page</label>
                                <div className="grid grid-cols-5 gap-2">
                                    {[10, 20, 30, 50, 100].map(value => (
                                        <button
                                            ...
                                            className={`px-4 py-3 rounded-xl text-sm font-bold transition-all ${itemsPerPage === value
                            ...
                            <button
                                className="w-full h-16 bg-blue-500 text-white font-black uppercase tracking-[0.2em] text-xs rounded-[1.5rem] flex items-center justify-center gap-4 hover:scale-[1.02] shadow-2xl shadow-blue-500/30 active:scale-[0.98] transition-all cursor-pointer"
                                onClick={onSave}
                            >
                                <Save size={18} />
                                Confirm & Sync
